function expert_params = moe_train_featsel(data,targets,num_experts,max_iter)
...
while iter_counter < max_iter
	...
	if mod(iter_counter,2) == 0 % doing if for the individual experts
		aff_expert = randi(K);
		aff_expert_w_featsel = expert_params{affected_classifier}.w_featsel;
		switch_vector = [rand(1,D-1) 1 ] < prob_simannl; % last 1 for the bias term, should never be 0.			
		...
		expert_params_new{affected_classifeir}.w_featsel = aff_expert_w_featsel_new;
	else % doing if for the feature space dividers
		aff_expert = randi(K);
		aff_expert_v_featsel = expert_params{affected_classifier}.v_featsel;
		switch_vector = [rand(1,D-1) 1 ] < prob_simannl; % last 1 for the bias term, should never be 0.
		aff_expert_v_featsel_new = xor(aff_expert_v_featsel,switch_vector);

		expert_params_old = expert_params;
		expert_params_new = expert_params;
		expert_params_new{affected_classifeir}.v_featsel = aff_expert_v_featsel_new;
	...
	for i = 1:2
		...
		if i == 1
	        	[expert_params_old, all_likelihood_old] = e_step_featsel(expert_params_cur,N,K);
		else
			[expert_params_new, all_likelihood_new] = e_step_featsel(expert_params_cur,N,K);
		end

	end

	if all_likelihood_old > all_likelihood_new
